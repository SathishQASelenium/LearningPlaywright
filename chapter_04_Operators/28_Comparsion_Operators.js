// let x = 10;
// x -= 3; // x = x-3;
// console.log(x);

// Compasion Operator (will always result in the boolean, true or false)

// =, ==, ===

// = -> Assignment opearator
// == -> loose comparsion ( sikh vs hindu )
// === -> strict comparsion ( sikh vs hindu , languge, living)

// > , < , >= , <= , !, !== , 

console.log(`3 > 4`, 3 > 4);
console.log(`3 < 4`, 3 < 4);
console.log(`4 >= 4`, 4 >= 4); // 4 > 4 or 4===4 , or gate
console.log(`3 <= 4`, 3 <= 4);

console.log(`5 == "5"`, 5 == "5"); // lose couple comparsion
console.log(`5 === "5"`, 5 === "5"); // stict not allowed

console.log(`5 != "5"`, 5 != "5"); // false , 5 = int, "5" string, both of them are not equal? - lose couple
console.log(`5 !== "5"`, 5 !== "5"); // true
//console.log(5 !=== "5"); This doesn't excit

// === Strict check we will check for both the datatype and value
// == Lose check we will check either value or data type.

// >== No